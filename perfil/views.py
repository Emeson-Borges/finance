...
def cadastrar_banco(request):
  ...
  if len(apelido.strip()) == 0 or len(valor.strip()) == 0:
    # Função de mensagem recebe três parâmetros (requisição, tipo da msg, texto da msg)
  ...
  #Instaciando a Models Conta na variavel conta
  conta = Conta(
    apelido=apelido,
    banco=banco,
    tipo=tipo,
  ...
  #Passando a função Save() na variavel para salvar no banco
  conta.save()
...
def update_categoria(request, id):
  ...
  #messages.add_message(request, constants.SUCCESS, 'Essencialidade alterada com sucesso')
  return redirect('/perfil/gerenciar/')
